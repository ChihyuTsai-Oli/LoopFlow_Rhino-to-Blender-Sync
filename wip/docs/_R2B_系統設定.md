...
- **執行時設定與即時檔**：跟目前 Rhino 工作檔所在資料夾走，寫進 `_LoopFlow_Config/loopflow_R2B/`（與 LoopFlow 的 `_LoopFlow_Config/loopflow/` 同層）。未存檔即停止。確切檔名與 schema 仍待資料契約盤點。
- 已發布 2.x 仍讀 `%APPDATA%\McNeel\Rhinoceros\8.0\scripts\LoopFlow_R2B\Data\`；3.0 發布時一次切換，不改現行安裝。
- 測檔／素材環境變數：`LOOPFLOW_R2B_WORKFILES_ROOT`（雙機實際路徑見工作區 `工作檔路徑.md`）。**不再**把 Dropbox `exchange/` 當執行時 JSON 根目錄。
- 即時檔不提交 Git。

3.0 producer／consumer 不寫死公司或家中的絕對路徑。語系等電腦偏好若需要，另放 `%APPDATA%`，不進專案 `_LoopFlow_Config`。

## 3.0 開發模式

- `main`、`v2.0.0` 與既有 `releases/` 作為舊版／fork 基準，不在重構過程逐支改造成半新半舊系統。
- 3.0 在新的 `wip/src/`、隔離 Rhino 安裝與 Blender profile／package ID 中乾淨建立，正式發布時一次切換。
...
目前 repo 內約 21 支 Python，包含 Rhino producer、importer fork 與 Toolkit。`releases/` 同時承擔 source 與 payload；只有在 3.0 build 管線完成後才切換 `wip/src/` 為唯一來源。

## 重構期間的 Rhino 測試入口

完整指令名稱與可貼上的按鈕巨集見 **`wip/docs/rhino指令.md`**（SSOT）。路徑或指令增減時先改該檔。

摘要：測試按鈕指向 `wip/src/rhino/entrypoints\`（檔案尚未建立）。暫定指令：`R2B_Models`、`R2B_Camera`、`R2B_Light`、`R2B_Open`。這不是已凍結的 3.0 contract；裁決見 `前期規劃/資料生態決策表_三家建議.md`。
