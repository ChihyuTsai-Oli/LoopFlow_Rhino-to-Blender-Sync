...
本機封裝用，**不是** `v3.0.0` 上架包。版號 `0.1.4`。yak 含 Blender Sync zip（`templates/`）；第一次跑任一產品指令拷到「文件\LoopFlow\Rhino to Blender Sync」。zip 內層資料夾與 extension id 同為 `loopflow_r2b_sync`。工具列 RUI 進包時改成與 `.rhp` 同名，Rhino 才會自動載入。
